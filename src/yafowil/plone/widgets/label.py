...
def label_renderer(widget, data):
    ...
    if (
        widget.attrs.get(widget.attrs["required_bullet_trigger"])
        and data.mode == "edit"
    ):
        label_contents.append(
            data.tag(
                "span", "&nbsp;", class_="required", title=_("required", "Required")
            )
        )
    label_contents.append(data.tag("span", help_text, class_="formHelp"))
    rendered = data.rendered is not UNSET and data.rendered or ""
    position = widget.attrs["position"]
    if callable(position):
        position = position(widget, data)
    combined_label = " ".join(label_contents)
...
factory.register(
    "plonelabel", edit_renderers=[label_renderer], display_renderers=[label_renderer]
)
...
factory.doc["props"][
    "plonelabel.required_bullet_trigger"
] = """\
